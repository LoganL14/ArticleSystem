...
def load_meta_file(meta_file: str):
    """ Read the metadata jsonl file to later get chunk texts """
...
def get_top5_embeddings(embed_file: str) -> list[tuple[int,float]]:
    """ Get the top 5 embeddings closest to centroid for an article """
...
def top_texts(meta_file: str, top5: list[tuple[int,float]]) -> list[str]:
    """ Get the chunk texts for the top 5 embeddings """
...
def save_summary_prompt(prompt: str, article_id: str, start_utc_time: str):
    """ Save the summary prompt to SUMMARY_PROMPTS_ROOT / start_utc_time / article_id.md """
    summary_prompt_folder = Path(SUMMARY_PROMPTS_ROOT) / start_utc_time    
